...
function updateMediaInputs() {
    ...
    const metricOptions = [
        '', '광고 비용', '노출 수', '클릭 수', '전환 수', '광고 매출',
        '영상 조회 수', '완료 시청 수', '도달 수'
    ];

    for (let i = 0; i < mediaCount; i++) {
        const mediaSection = document.createElement('div');
        mediaSection.id = `mediaSection${i}`;
        mediaSection.className = 'media-section';
        mediaSection.innerHTML = `
            <h2>매체 ${i + 1}</h2>
            <input type="text" id="mediaName${i}" placeholder="매체 이름 입력 (선택 사항)">
            <div id="metricRows${i}" class="metric-rows"></div>
            <button onclick="addMetricRow(${i})">지표 추가</button>
        `;
        mediaInputs.appendChild(mediaSection);
        addMetricRow(i); // 기본적으로 한 줄 추가
    }
}

function addMetricRow(mediaIndex) {
    const metricRows = document.getElementById(`metricRows${mediaIndex}`);
    const row = document.createElement('div');
    row.className = 'metric-row';
    row.innerHTML = `
        <select onchange="calculateMetrics()">
            <option value="">지표 선택</option>
            <option value="광고 비용">광고 비용</option>
            <option value="노출 수">노출 수</option>
            <option value="클릭 수">클릭 수</option>
            <option value="전환 수">전환 수</option>
            <option value="광고 매출">광고 매출</option>
            <option value="영상 조회 수">영상 조회 수</option>
            <option value="완료 시청 수">완료 시청 수</option>
            <option value="도달 수">도달 수</option>
        </select>
        <input type="text" placeholder="값 입력" oninput="formatNumber(this); calculateMetrics()">
        <button onclick="this.parentElement.remove(); calculateMetrics()">삭제</button>
    `;
...
function showGoogleAdsMetrics() {
    alert("광고 지표 설명:\n\n" +
        "CPM: 1,000회 노출당 비용 (광고 비용 / 노출 수 * 1000)\n" +
        "CPC: 클릭 1회당 비용 (광고 비용 / 클릭 수)\n" +
        "CTR: 클릭률 (클릭 수 / 노출 수 * 100)\n" +
        "CPA: 전환 1건당 비용 (광고 비용 / 전환 수)\n" +
        "ROAS: 광고비 대비 매출 비율 (광고 매출 / 광고 비용 * 100)\n" +
        "CPV: 영상 조회 1회당 비용 (광고 비용 / 영상 조회 수)\n" +
        "VTR: 영상 완료 시청 비율 (완료 시청 수 / 노출 수 * 100)\n" +
        "평균 빈도: 사용자당 평균 노출 횟수 (노출 수 / 도달 수)\n" +
        "Reach1+: 1회 이상 노출된 고유 사용자 수\n" +
        "Reach3+: 3회 이상 노출된 고유 사용자 수\n" +
        "GRP: 도달률과 빈도의 총합 (노출 수 / 도달 수 * 100)\n" +
        "CPRP: GRP 1점당 비용 (광고 비용 / GRP)");
}